...
public class InitializerCfg {
    ...
    public CommandLineRunner dbInitializer() {
        return args -> {
            ...
            Gateway gateway0 = null;
            Gateway gateway1 = null;
            try {
                gateway0 = new Gateway("gw0", "gateway0", "10.8.6.50");
//                gateway0.getDevices().addAll(Arrays.asList(
//                        new Device(1, "Sony", Date.from(Instant.now()), DeviceStatus.ONLINE),
//                        new Device(2, "Apple", Date.from(Instant.now()), DeviceStatus.ONLINE),
//                        new Device(3, "Cisco", Date.from(Instant.now()), DeviceStatus.ONLINE),
//                        new Device(4, "Huawei", Date.from(Instant.now()), DeviceStatus.ONLINE),
//                        new Device(5, "Panasonic", Date.from(Instant.now()), DeviceStatus.ONLINE),
//                        new Device(6, "LG", Date.from(Instant.now()), DeviceStatus.ONLINE),
//                        new Device(7, "Samsung", Date.from(Instant.now()), DeviceStatus.ONLINE),
//                        new Device(8, "Logitech", Date.from(Instant.now()), DeviceStatus.ONLINE),
//                        new Device(9, "TP-Link", Date.from(Instant.now()), DeviceStatus.ONLINE),
//                        new Device(10, "Hawlett-Packard", Date.from(Instant.now()), DeviceStatus.ONLINE)));
                gateway1 = new Gateway("gw1", "gateway0", "10.8.6.51");
                ...
                logger.error("Error initializing gateways", e);
            }
